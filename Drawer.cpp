...
void DrawEdges(int a, int b)
{
    glBlendFunc(GL_DST_ALPHA,GL_ONE_MINUS_DST_ALPHA);
    glBegin(GL_LINES);
    glVertex2f(points[a].first+512, points[a].second+512);
    glVertex2f(points[b].first+512, points[b].second+512);
    glEnd();
}
...
void display()
{
    ...
    for (int i = 0; i < points.size(); ++i){
        ...
        DrawPoints(points[i].first + 512, points[i].second + 512);
    }
...
void AnswerVisualizer(vector<std::pair<double, double>> &data, vector<std::pair<int, int>> &edges, int &DotsNum)
{
    points = data;
    tree = edges;
    DotsNumber = DotsNum;
...
void SpanningTreeVisualizer(vector<std::pair<double, double>> &data, vector<std::vector<std::pair<int, int>>> &edges, int &DotsNum)
{
    points = data;
    MinTree = edges;
    DotsNumber = DotsNum;
